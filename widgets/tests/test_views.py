...
class TestCommentCreateView(TestDataMixin, TestCase):
    ...
    def test_correct_template(self):
        ...
        self.assertTemplateUsed(response, "widgets/comment_form.html")
...
class TestCommentUpdateView(TestDataMixin, TestCase):
    def setUp(self):
        self.comment = self.widget1.comments.create(
            user=self.commenter, comment=random_string(125)
        )
    ...
    def test_message_after_submit(self):
        ...
        response = self.post_client.post(
            self.url, data={"comment": comment}, follow=True
        )
        messages = list(response.context["messages"])
    ...
    def test_form_submit_failure(self):
        response = self.post_client.post(
            self.url, data={"comment": random_string(1025)}
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "The comment is too long.")
